...
const char MONOPOLYMONEY_URI_SCHEME_NAME[] = "monopolymoney";
...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("MonopolyMoneywallet");
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("MonopolyMoneywallet.lock"));
  QUrl paymentUrl = QUrl::fromUserInput(arguments().last());
  if (paymentUrl.scheme() != MONOPOLYMONEY_URI_SCHEME_NAME) {
    paymentUrl = QUrl();
  }
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] MonopolyMoney wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "MonopolyMoney wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have MonopolyMoneyd with non-default RPC port, you should terminate it and relaunch MonopolyMoneyWallet\n"
      "or\n"
      "Set the Local deamon required port in MonopolyMoneyWallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
