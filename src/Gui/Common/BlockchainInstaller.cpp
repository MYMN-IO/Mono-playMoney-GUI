...
const quint32 MONOPOLYMONEY_BLOCK_SIZE = 0xd5;
...
BlockchainInstaller::BlockchainInstaller(QObject* _parent) : QObject(_parent), m_blockIndexesFileName("blockindexes.dat"), m_blocksFileName("blocks.dat"),
  m_monopolymoneyDir(Settings::instance().getDataDir().absolutePath()), m_applicationDir(QDir::current()) {
}
...
void BlockchainInstaller::exec() {
  ...
  if (!checkIfMonopolyMoneyBlockchainExists()) {
    installBlockchain();
    return;
  }

  quint64 currentHeight;
  quint64 monopolymoneyHeight;
  if (!checkIfBlockchainOutdated(currentHeight, monopolymoneyHeight)) {
    return;
  }
  ...
  QuestionDialog dlg(tr("Blockchain installation"), QString(questionStringTemplate).arg(monopolymoneyHeight).arg(currentHeight), nullptr);
...
bool BlockchainInstaller::getGenesisBlockFromBlockchain(char** _genesisBlockData) const {
  ...
  if (blockSize != MONOPOLYMONEY_BLOCK_SIZE) {
    return false;
  }
...
bool BlockchainInstaller::checkIfMonopolyMoneyBlockchainExists() const {
  return m_monopolymoneyDir.exists() && m_monopolymoneyDir.exists(m_blocksFileName);
}
...
bool BlockchainInstaller::checkIfBlockchainOutdated(quint64& _sourceHeight, quint64& _targetHeight) const {
  ...
  QFile targetBlockIndexesFile(m_monopolymoneyDir.absoluteFilePath(m_blockIndexesFileName));
...
QFileInfo BlockchainInstaller::monopolymoneyBlockchainInfo() const {
  return QFileInfo(m_monopolymoneyDir.absoluteFilePath(m_blocksFileName));
}
...
void BlockchainInstaller::installBlockchain() {
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blockIndexesFileName),
    m_monopolymoneyDir.absoluteFilePath(m_blockIndexesFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blocksFileName), m_monopolymoneyDir.absoluteFilePath(m_blocksFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
