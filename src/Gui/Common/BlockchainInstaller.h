...
private:
  ...
  const QDir m_monopolymoneyDir;
  const QDir m_applicationDir;

  bool checkIfCurrentBlockchainExists() const;
  bool checkIfMonopolyMoneyBlockchainExists() const;
  bool getGenesisBlockFromBlockchain(char** _genesisBlockData) const;
  bool checkIfBlockchainOutdated(quint64& _current, quint64& _monopolymoney) const;
  QFileInfo currentBlockchainInfo() const;
  QFileInfo monopolymoneyBlockchainInfo() const;
