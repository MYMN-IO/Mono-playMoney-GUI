...
const char DEFAULT_WALLET_FILE_NAME[] = "monopolymoneywallet.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  //m_defaultPoolList << "pool.mymn.io:3333";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("monopolymoneywallet.cfg"));
...
QStringList Settings::getDefaultMiningPoolList() const {
  return m_defaultPoolList;
}
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("monopolymoneywallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("monopolymoneywallet.desktop");
  ...
  res = autorunSettings.contains("MonopolyMoneyWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("MonopolyMoneyWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.monopolymoney.monopolymoneywallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=MonopolyMoney Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("MonopolyMoneyWallet", appPath);
    } else {
      autorunSettings.remove("MonopolyMoneyWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\monopolymoney", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:monopolymoney");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\monopolymoney\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\monopolymoney\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("monopolymoneywallet.cfg"));
